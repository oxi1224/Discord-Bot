 ...
 * @param {string} [args.color=config.config.colors.base] - Color of the embed.
...
export function createReplyEmbed(args) {
  ...
  const color = !args || !args.color ? config.colors.base : args.color;
...
export const dmDuration = (action, guild, reason, duration) => createReplyEmbed({
  title: `You've been ${!duration ? `permanently ${action}` : `${action} for ${duration}`} in ${guild}.`,
  description: `Reason: \`\`${reason}\`\`.`,
});
