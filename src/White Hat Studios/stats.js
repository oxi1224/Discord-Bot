...
async function playerCount(guild) {
  ...
  const channel = await guild.channels.fetch(config.guildId.statsChannels.players);
...
async function guildMembers(guild) {
  const channel = await guild.channels.fetch(config.guildId.statsChannels.guildMembers);
...
async function fansCount(guild) {
  ...
  const channel = await guild.channels.fetch(config.guildId.statsChannels.groupMembers);
